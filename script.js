...
idx = comments.findIndex((ind)=>{
    return ind.id === 123523;
})
...
totalYearsLived = inventors.reduce(function(acc, curr) {
...
travelMethodCounts = travelMethods.reduce(function(acc,curr){
  if(acc[curr]){
    acc[curr] ++
  }else{
    acc[curr] = 1
